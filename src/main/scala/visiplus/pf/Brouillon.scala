...
object Main {
  ...
  def main(args: Array[String]): Unit = println("toto")
}
...
def calculateTotal(products: List[Product]): Double = products.map(_.total).sum
...
val toto = {
  ...
  val average_1: List[Int] => Double = divide compose bimap(sum)(count[Int]) compose duplicate[List[Int]]
...
case class Bill(productLines: Seq[ProductLine]) {
  def total: Double = productLines.map(p => p.costPerItem * p.quantity).sum
}
...
@main def animalMain(): Unit = println(Animal.Dog("felix").fullName) //felix (Dog)
...
object TryEtTry {
  ...
  object avecTry {
    ...
    def divide(numerator: Int, denominator: Int): Try[Int] = Try(numerator / denominator)
...
val xxxxxx = AAA.unapply(AAA("a", "b"))
