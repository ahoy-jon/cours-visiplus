...
def calculateVAT(amount: BigDecimal): BigDecimal = {
  (amount * BigDecimal(0.2)).setScale(2, RoundingMode.HALF_UP)
}
...
def calcVAT():Unit = 
  println(s"Montant : $amount, TVA : $vat")
...
object TypesExercises {
  ...
  val valeursTupleBoolBool: Set[(Boolean, Boolean)] = for {
    b1 <- valeursBool
    b2 <- valeursBool
  } yield (b1, b2)

  // Exercice 6 : Listez toutes les valeurs du type `Either[Either[Unit, Unit], Unit]`
  val valeursEitherEitherUnitUnitUnit: Set[Either[Either[Unit, Unit], Unit]] = Set(
    Left(Left(())), Left(Right(())), Right(())
  )

  // Exercice 7 : Donnez les éléments dans `A * B` avec `A = { true, false }` et `B = { "red", "green", "blue" }`
  val produitAB: Set[(Boolean, String)] = for {
    b <- valeursBool
